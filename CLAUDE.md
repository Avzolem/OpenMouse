...
4. `Tray` (`tray.py`) — pystray icon with status text, Quit, and Uninstall menu items. Runs in its own thread; communicates back via callbacks (`on_quit`, `on_uninstall`). Imported lazily inside `run_server` and entirely optional — on a desktop without GTK/appindicator `import pystray` raises, and that must not stop the server. On Linux the menu only works with the `appindicator` (or `gtk`) backend: pystray's `xorg` backend has `HAS_MENU = False`, so a click just runs the default item and this menu has none — the icon looks alive and does nothing. `Tray.start` logs `System tray started (backend: <name>)` and warns on `xorg`.
...
The Linux binaries bundle PyGObject, Gtk 3 and AyatanaAppIndicator3 so the tray gets a real menu. Three things keep that from silently regressing to the menu-less `xorg` backend:

- `openmouse.spec` lists the pystray backends and `gi.repository` modules by hand. The contrib `hook-pystray` uses `collect_submodules("pystray")`, which *imports* each backend at build time; `pystray._util.gtk` calls `Gtk.init_check()` on import and fails without a display, so `appindicator`/`gtk` used to drop out without any error. The spec also aborts the build if `gi` or the typelibs are missing.
- The Linux CI jobs install `libgirepository1.0-dev libcairo2-dev gir1.2-gtk-3.0 gir1.2-ayatanaappindicator3-0.1` and pin `PyGObject>=3.50,<3.52`: 3.52 moved to `girepository-2.0` (GLib ≥ 2.80), which the `ubuntu-22.04` base (GLib 2.72) does not have. PyGObject is not in `requirements.txt` because it does not install on Windows.
- The smoke test runs under `xvfb-run -a dbus-run-session` and greps for `backend: appindicator`, not just `Listening`.
